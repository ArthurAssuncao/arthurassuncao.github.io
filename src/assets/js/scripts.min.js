...
function show_skills_itens() {
    $("#cv-habilidades ul > li:gt(11)").show(1e3);
}
...
function show_prod_cientifica_itens() {
    $("#cv-producao ul > li:gt(0)").show(1e3);
}

function show_habilidades_itens() {
    show_skills_itens(), show_prod_cientifica_itens(), $("#btn-skill-more").hide(900);
}

function hide_habilidades_itens() {
    hide_skills_itens(), hide_prod_cientifica_itens();
}
...
function on_scroll_menu(event) {
    var navbarHeight = $("#navbar").height(), scrollPos = $(document).scrollTop() + navbarHeight;
    $("#navbar a").each(function() {
        var currLi = $(this), currLink = $(this), refElement = $(currLink.attr("href")), menuSide = $('.menu_left_side li[data-href="' + currLink.attr("href") + '"]');
        refElement.position().top <= scrollPos && refElement.position().top + refElement.height() > scrollPos ? ($("#navbar a").removeClass("active"), 
        $(".menu_left_side ul li").removeClass("active"), currLi.addClass("active"), menuSide.addClass("active")) : (currLi.removeClass("active"), 
        menuSide.removeClass("active"));
    });
    var has_item_active = !1;
    $("#navbar a").each(function() {
        var currLi = $(this);
        currLi.hasClass("active") && (has_item_active = !0);
    }), has_item_active ? $("#nav_f").addClass("nav-shadow") : $("#nav_f").removeClass("nav-shadow");
}

function msg_send(success) {
    success ? ($("#form-contact").addClass("animated fadeOutRight"), $("#form-contact").hide(500), 
    $(".form-result-ok").show().addClass("animated fadeInLeft"), $(".form-result-ok").attr("position", "relative")) : ($("#form-contact").addClass("animated fadeOutLeft"), 
    $("#form-contact").hide(500), $(".form-result-error").show().addClass("animated fadeInRight"), 
    $(".form-result-error").attr("position", "relative"));
}

function hide_form_result() {
    $(".form-result-ok").hide(), $(".form-result-error").hide();
}
...
app.config(function($mdThemingProvider) {
    ...
    function sanitizePosition() {
        ...
        current.bottom && last.top && (current.top = !1), current.top && last.bottom && (current.bottom = !1), 
        current.right && last.left && (current.left = !1), current.left && last.right && (current.right = !1), 
    ...
    $rootScope.toastPosition = angular.extend({}, last), $rootScope.getToastPosition = function() {
        return sanitizePosition(), Object.keys($rootScope.toastPosition).filter(function(pos) {
            return $rootScope.toastPosition[pos];
        }).join(" ");
    }, this.showSimpleToast = function(msg) {
        $mdToast.show($mdToast.simple().content(msg).position($rootScope.getToastPosition()).hideDelay(3e3));
    }, this.showSimpleToastTimeMillis = function(msg, time) {
        $mdToast.show($mdToast.simple().content(msg).position($rootScope.getToastPosition()).hideDelay(time));
    };
}), // Directiva Skill
app.directive("skill", function() {
    ...
    $scope.levels = [ {
        ...
        return value >= $scope.levels[3].value_min ? $scope.levels[3].name : value >= $scope.levels[2].value_min ? $scope.levels[2].name : value >= $scope.levels[1].value_min ? $scope.levels[1].name : value >= $scope.levels[0].value_min ? $scope.levels[0].name : "indefinido";
    };
}), // Directiva paper
app.directive("paper", function() {
    var directive = {};
    // directive.replace = true;
    return directive.restrict = "E", directive.controller = "PaperController", directive.scope = {
        url: "@url",
        title: "@title",
        authors: "@authors",
    ...
    }, directive.templateUrl = "../../templates/paper.tmpl.html", directive;
...
app.directive("course", function() {
    ...
    return directive.restrict = "E", directive.scope = {
        ...
        date_end: "@dateend"
    }, directive.controller = "CourseController", directive.templateUrl = "../../templates/course.tmpl.html", 
    directive;
}), app.controller("CourseController", function($scope) {}), // Directiva project
app.directive("project", function($templateCache) {
    ...
    return directive.restrict = "E", directive.scope = {
    ...
    }, directive.controller = "ProjectController", directive.controllerAs = "pc", directive.bindToController = {
        name: "@name",
        fullname: "@fullname",
        imgurl: "@imgurl",
        imgs: "@imgs",
        ...
        dateend: "@dateend",
        description: "@description"
    }, directive.templateUrl = "../../templates/project.tmpl.html", $templateCache.put("../../templates/project-dialog.tmpl.html"), 
    directive;
}), app.controller("ProjectController", function($scope, $mdMedia, $mdDialog) {
    function DialogProjectController($scope, $mdDialog, project) {
        $scope.project = project, $scope.split = function(text) {
            return text.split(",");
    ...
    var project = {};
    project.name = this.name, project.fullname = this.fullname, project.imgurl = this.imgurl, 
    project.imgs = this.imgs, project.url = this.url, project.urlname = this.urlname, 
    project.technologies = this.technologies, project.type = this.type, project.datestart = this.datestart, 
    project.dateend = this.dateend, project.description = this.description, $scope.showDialog = function(ev) {
...
app.directive("award", function() {
    ...
    return directive.restrict = "E", directive.scope = {
        ...
        place: "@place",
        url: "@url"
    }, directive.controller = "AwardController", directive.templateUrl = "../../templates/award.tmpl.html", 
    directive;
}), app.controller("AwardController", function($scope) {}), // AngularJS controllers
app.controller("AppController", function($scope, $timeout, $mdSidenav, $log, toastService) {
    $scope.openSideMenu = function() {
        $mdSidenav("left").toggle().then(function() {
            $log.debug("abrir menu lateral");
        });
    }, toastService.showSimpleToastTimeMillis("Versão beta, alguns erros ainda podem aparecer", 6e4);
}), app.controller("TyperController", function($scope) {}), app.controller("ParallaxController", function($scope, parallaxHelper) {
    $scope.background = parallaxHelper.createAnimator(-.5);
    ...
    // cria a lista
    $scope.skills = {}, $scope.skills.data = [ {
        name: "AngularJS",
        value: 60,
        msg: "Utilizei em um projeto vencedor do hackathon GDG, neste site pessoal e em um app que está em desenvolvimento, além de um curso de front-end."
    }, {
        name: "NodeJS",
        value: 55,
        msg: "Utilizei em um projeto vencedor de hackathon, no projeto JUMP! e em um app que está em desenvolvimento, além de um curso de MEAN."
    }, {
        name: "MongoDB",
        value: 45,
        msg: "Utilizei em um projeto vencedor de hackathon e em um app que está em desenvolvimento, além de um curso de MEAN."
    }, {
        name: "JavaScript/jQuery",
        value: 75,
        msg: "Trabalhei em alguns sites, extensões para o Chrome e projetos."
    }, {
        name: "HTML5/CSS3/SASS",
        value: 80,
        msg: "Trabalhei em alguns sites, extensões para o Chrome e alguns projetos."
    }, {
        name: "ReactJS",
        value: 15,
        msg: "Aprendi durante um curso de Front-end."
    }, {
        name: "Git (Controle de Versão)",
        value: 90,
        msg: "Utilizo Git desde 2012 em boa parte dos projetos que participo, além de ter trabalhado por 1,25 ano e de utilizar no mestrado."
    }, {
        name: "Design Responsivo",
        value: 70,
        msg: "Trabalhei em alguns sites com Design Responsivo."
    }, {
        name: "MaterializeCSS / Bootstrap 2.x-3.x",
        value: 70,
        msg: "Trabalhei em alguns sites utilizando Bootstrap e MaterializeCSS, como as versões anteriores deste site pessoal, extensões para o Chrome e projetos."
    }, {
        name: "Grunt (automatizador)",
        value: 60,
        msg: "Utilizo neste site e em um app que está em desenvolvimento."
    }, {
        name: "Android",
        value: 70,
        msg: "Trabalhei por 1,25 ano e fiz um curso de Android, além de alguns projetos."
    }, {
        name: "Python",
        value: 60,
        msg: "Trabalhei por 1,25 ano, fiz um site e também um curso de Python, além de vários projetos, inclusive no mestrado."
    }, {
        name: "R",
        value: 70,
        msg: "Utilizo em projetos do mestrado."
    ...
    // { name: 'C++', value: 50, msg: 'Participei de projetos utilizando C++ e utilizei no mestrado.' },
    {
        name: "SQL",
        value: 60,
        msg: "Trabalhei por 1,5 ano utilizando SQL, com o MySQL(1,25 ano) e Postgree(0,25 ano)."
    }, {
        name: "Objective-C (Programação para iOS)",
        value: 15,
        msg: "Aprendi no mestrado cursando uma disciplina de desenvolvimento móvel."
    }, {
        name: "PHP",
        value: 40,
        msg: "Trabalhei por alguns meses, além de criar as versões anteriores deste site e alguns projetos em PHP."
    } ];
}), app.controller("PapersController", function($scope) {
    $scope.sortType = "value", // sort type default
    $scope.sortReverse = !1, // ordenacao do sort
    // cria a lista
    $scope.papers = {}, $scope.papers.data = [ {
    ...
    $scope.sortReverse = !1, // ordenacao do sort
    // cria a lista
    $scope.awards = {}, $scope.awards.data = [ {
    ...
    $scope.sortReverse = !1, // ordenacao do sort
    // cria a lista
    $scope.projects = {}, $scope.projects.data = [ {
        name: "EntregaRápida",
    ...
    remove_form_send_by_enter(), hide_form_result(), hide_habilidades_itens(), fill_skills(), 
    add_habilidades_more_event(), iniciar_wow(), add_contact_form_click(), $(window).scroll(on_scroll_menu), 
    iniciar_scrollspy();
}), /**
...
function($) {
    ...
    function onScroll() {
        ...
        ++ticks;
        // viewport rectangle
        var top = jWindow.scrollTop(), left = jWindow.scrollLeft(), right = left + jWindow.width(), bottom = top + jWindow.height(), intersections = findElements(top + offset.top, right + offset.right, bottom + offset.bottom, left + offset.left);
        $.each(intersections, function(i, element) {
            var lastTick = element.data("scrollSpy:ticks");
            "number" != typeof lastTick && // entered into view
            element.triggerHandler("scrollSpy:enter"), // update tick id
            element.data("scrollSpy:ticks", ticks);
        }), // determine which elements are no longer in view
        $.each(elementsInView, function(i, element) {
            var lastTick = element.data("scrollSpy:ticks");
            "number" == typeof lastTick && lastTick !== ticks && (// exited from view
            element.triggerHandler("scrollSpy:exit"), element.data("scrollSpy:ticks", null));
